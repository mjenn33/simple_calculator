    ...
    return Scaffold(
      ...
      body: SafeArea(child: Center(child: ConstrainedBox(
        ...
        child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
          const Padding(padding: EdgeInsets.fromLTRB(4, 12, 4, 20), child: Row(children: [Icon(Icons.calculate_rounded, color: Color(0xFF3257D5)), SizedBox(width: 10), Text('Simple Calculator', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: navy))])),
          Expanded(child: Container(width: double.infinity, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: navy, borderRadius: BorderRadius.circular(28)), child: Column(children: [
            Expanded(child: Align(alignment: Alignment.bottomRight, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerRight, child: Text(_display, style: const TextStyle(color: Colors.white, fontSize: 56, fontWeight: FontWeight.w300))),
          ...
          const Padding(padding: EdgeInsets.only(top: 16), child: Text('Basic addition, subtraction, multiplication, and division', style: TextStyle(color: Color(0xFF667085)))),
        ])),
      ))),
    );
  }
  Widget _row(List<_Key> keys) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [for (final key in keys) Expanded(flex: key.flex, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 5), child: _CalculatorButton(data: key)))]));
}
...
class _CalculatorButton extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Semantics(button: true, label: data.label, child: SizedBox(height: 58, child: FilledButton(onPressed: data.onTap, style: FilledButton.styleFrom(backgroundColor: fill, foregroundColor: text, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: Text(data.label, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w600)))));
  }
}
